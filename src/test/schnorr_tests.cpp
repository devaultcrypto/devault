...
BOOST_AUTO_TEST_CASE(opcodes_random_flags) {
    ...
    for (int i = 0; i < 4096; i++) {
        ...
        if (hasSchnorr) {
            ...
            if (hasStricts) {
                CheckError(flags, {{}, Zero64_with_hashtype, {1}, pubkeyC, {1}},
                           scriptCHECKMULTISIG, ScriptError::SIG_DER);
                CheckError(flags, {{}, Zero64_with_hashtype, {1}, pubkeyC, {1}},
                           scriptCHECKMULTISIGVERIFY, ScriptError::SIG_DER);
            } else if (hasNullFail) {
                CheckError(flags, {{}, Zero64_with_hashtype, {1}, pubkeyC, {1}},
                           scriptCHECKMULTISIG, ScriptError::SIG_NULLFAIL);
                CheckError(flags, {{}, Zero64_with_hashtype, {1}, pubkeyC, {1}},
                           scriptCHECKMULTISIGVERIFY, ScriptError::SIG_NULLFAIL);
            } else {
