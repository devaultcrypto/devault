...
of 10 * MAX_TX_SIZE. */
TEST_CASE("validation_load_external_block_file") {
  TestingSetup setup;
  fs::path tmpfile_name =
      setup.pathTemp / strprintf("vlebf_test_%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(100000)));
  ...
  size_t nwritten = fwrite(std::begin(chainparams.DiskMagic()), CMessageHeader::MESSAGE_START_SIZE, 1, fp);
  ...
  BOOST_CHECK(GetSerializeSize(block, SER_DISK, CLIENT_VERSION) > 2 * MAX_TX_SIZE);
