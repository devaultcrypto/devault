...
TEST_CASE("util_ParseHex") {
  ...
  std::vector<uint8_t> expected(ParseHex_expected, ParseHex_expected + sizeof(ParseHex_expected));
  // Basic test vector
  ...
  int i = 0;
  for (auto r : result) {
    BOOST_CHECK_EQUAL(r, expected[i++]);
  }
...
TEST_CASE("util_HexStr") {
  ...
  BOOST_CHECK_EQUAL(
      HexStr(ParseHex_expected + sizeof(ParseHex_expected), ParseHex_expected + sizeof(ParseHex_expected)), "");

  BOOST_CHECK_EQUAL(
      HexStr(ParseHex_expected + sizeof(ParseHex_expected), ParseHex_expected + sizeof(ParseHex_expected), true), "");
  ...
  BOOST_CHECK_EQUAL(HexStr(ParseHex_vec.rbegin(), ParseHex_vec.rend()), "b0fd8a6704");

  BOOST_CHECK_EQUAL(HexStr(ParseHex_vec.rbegin(), ParseHex_vec.rend(), true), "b0 fd 8a 67 04");

  BOOST_CHECK_EQUAL(HexStr(std::reverse_iterator<const uint8_t *>(ParseHex_expected),
  ...
  BOOST_CHECK_EQUAL(HexStr(std::reverse_iterator<const uint8_t *>(ParseHex_expected + 1),
                           std::reverse_iterator<const uint8_t *>(ParseHex_expected)),
                    "04");
  ...
  BOOST_CHECK_EQUAL(HexStr(std::reverse_iterator<const uint8_t *>(ParseHex_expected + 5),
                           std::reverse_iterator<const uint8_t *>(ParseHex_expected)),
                    "b0fd8a6704");
  ...
  BOOST_CHECK_EQUAL(HexStr(std::reverse_iterator<const uint8_t *>(ParseHex_expected + 65),
                           std::reverse_iterator<const uint8_t *>(ParseHex_expected)),
...
TEST_CASE("util_ParseParameters") {
  ...
  BOOST_CHECK((testArgs.GetOverrideArgs().count("-a") && testArgs.GetOverrideArgs().count("-b") &&
               testArgs.GetOverrideArgs().count("-ccc") && !testArgs.GetOverrideArgs().count("f") &&
...
TEST_CASE("util_GetBoolArg") {
  ...
  BOOST_CHECK((testArgs.GetOverrideArgs().size() == 6 && testArgs.GetConfigArgs().empty()));
...
TEST_CASE("util_ReadConfigStream") {
  ...
  BOOST_CHECK((test_args.GetConfigArgs().count("-a") && test_args.GetConfigArgs().count("-b") &&
               test_args.GetConfigArgs().count("-ccc") && test_args.GetConfigArgs().count("-d") &&
               test_args.GetConfigArgs().count("-fff") && test_args.GetConfigArgs().count("-ggg") &&
               test_args.GetConfigArgs().count("-h") && test_args.GetConfigArgs().count("-i")));
  BOOST_CHECK((test_args.GetConfigArgs().count("-sec1.ccc") && test_args.GetConfigArgs().count("-sec1.h") &&
               test_args.GetConfigArgs().count("-sec2.ccc") && test_args.GetConfigArgs().count("-sec2.iii")));

  BOOST_CHECK((test_args.IsArgSet("-a") && test_args.IsArgSet("-b") && test_args.IsArgSet("-ccc") &&
               test_args.IsArgSet("-d") && test_args.IsArgSet("-fff") && test_args.IsArgSet("-ggg") &&
               test_args.IsArgSet("-h") && test_args.IsArgSet("-i") && !test_args.IsArgSet("-zzz") &&
               !test_args.IsArgSet("-iii")));

  BOOST_CHECK((test_args.GetArg("-a", "xxx") == "" && test_args.GetArg("-b", "xxx") == "1" &&
               test_args.GetArg("-ccc", "xxx") == "argument" && test_args.GetArg("-d", "xxx") == "e" &&
               test_args.GetArg("-fff", "xxx") == "0" && test_args.GetArg("-ggg", "xxx") == "1" &&
               test_args.GetArg("-h", "xxx") == "0" && test_args.GetArg("-i", "xxx") == "1" &&
               test_args.GetArg("-zzz", "xxx") == "xxx" && test_args.GetArg("-iii", "xxx") == "xxx"));

  for (bool def : {false, true}) {
    BOOST_CHECK((test_args.GetBoolArg("-a", def) && test_args.GetBoolArg("-b", def) &&
                 !test_args.GetBoolArg("-ccc", def) && !test_args.GetBoolArg("-d", def) &&
                 !test_args.GetBoolArg("-fff", def) && test_args.GetBoolArg("-ggg", def) &&
                 !test_args.GetBoolArg("-h", def) && test_args.GetBoolArg("-i", def) &&
                 test_args.GetBoolArg("-zzz", def) == def && test_args.GetBoolArg("-iii", def) == def));
  }
  ...
  BOOST_CHECK((test_args.GetArgs("-ccc").size() == 2 && test_args.GetArgs("-ccc").front() == "argument" &&
  ...
  int i = 0;
  for (auto s : sec1_ccc_res) {
    BOOST_CHECK_EQUAL(s, sec1_ccc_expected[i++]);
  }
  ...
  // same as original
  BOOST_CHECK((test_args.GetArg("-a", "xxx") == "" && test_args.GetArg("-b", "xxx") == "1" &&
               test_args.GetArg("-d", "xxx") == "e" && test_args.GetArg("-fff", "xxx") == "0" &&
               test_args.GetArg("-ggg", "xxx") == "1" && test_args.GetArg("-zzz", "xxx") == "xxx" &&
  ...
  i = 0;
  for (auto s : sec2_ccc_res) {
    BOOST_CHECK_EQUAL(s, sec2_ccc_expected[i++]);
  }
...
TEST_CASE("test_FormatParagraph") {
  ...
  BOOST_CHECK_EQUAL(FormatParagraph("This_is_a_very_long_test_string_without_any_spaces_so_"
  ...
  BOOST_CHECK_EQUAL(FormatParagraph("a b c d e f g h i j k l m n o p q r s t u v w x y z 1 "
  ...
  BOOST_CHECK_EQUAL(FormatParagraph("This is a very long test string. This is a second "
