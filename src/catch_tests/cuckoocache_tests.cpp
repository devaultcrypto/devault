 ...
 */
TEST_CASE("test_cuckoocache_no_fakes") {
 ...
 */
double normalize_hit_rate(double hits, double load) { return hits * std::max(load, 1.0); }

/** Check the hit rate on loads ranging from 0.1 to 2.0 */
TEST_CASE("cuckoocache_hit_rate_ok") {
  /**
...
TEST_CASE("cuckoocache_erase_ok") {
  size_t megabytes = 4;
...
TEST_CASE("cuckoocache_erase_parallel_ok") {
  size_t megabytes = 4;
...
TEST_CASE("cuckoocache_generations") { test_cache_generations<CuckooCache::cache<uint256, SignatureCacheHasher>>(); }
