...
TEST_CASE("cashaddr_testvectors_valid") {
...
TEST_CASE("cashaddr_testvectors_invalid") {
  static const std::string CASES[] = {
      "prefix:x32nx6hz",
      "prEfix:x64nx6hz",
      "prefix:x64nx6Hz",
...
TEST_CASE("cashaddr_rawencode") {
  ...
  int i = 0;
  for (auto t : toEncode.second) {
    BOOST_CHECK_EQUAL(t, decoded.second[i++]);
  }
}

TEST_CASE("cashaddr_testvectors_noprefix") {
