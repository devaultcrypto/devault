...
TEST_CASE("caddrdb_read") {
  ...
  Lookup("250.7.1.1", addr1, 33039, false);
  Lookup("250.7.2.2", addr2, 9999, false);
  Lookup("250.7.3.3", addr3, 9999, false);

  // Add three addresses to new table.
  CService source;
  Lookup("252.5.1.1", source, 33039, false);
  ...
  try {
    ...
    ssPeers1 >> addrman1;
  } catch (const std::exception &e) { exceptionThrown = true; }
...
TEST_CASE("caddrdb_read_corrupted") {
  BasicTestingSetup setup;
...
TEST_CASE("cnode_simple_test") {
  ...
  std::unique_ptr<CNode> pnode1(
      new CNode(id++, NODE_NETWORK, height, hSocket, addr, 0, 0, CAddress(), pszDest, fInboundIn));
  BOOST_CHECK(pnode1->fInbound == false);
  BOOST_CHECK(pnode1->fFeeler == false);

  fInboundIn = true;
  std::unique_ptr<CNode> pnode2(
      new CNode(id++, NODE_NETWORK, height, hSocket, addr, 1, 1, CAddress(), pszDest, fInboundIn));
  BOOST_CHECK(pnode2->fInbound == true);
  BOOST_CHECK(pnode2->fFeeler == false);
}

TEST_CASE("test_getSubVersionEB") {
  BasicTestingSetup setup;
...
TEST_CASE("test_userAgent") {
  ...
  const std::string versionMessage = "/DeVault Core:" + std::to_string(CLIENT_VERSION_MAJOR) + "." +
