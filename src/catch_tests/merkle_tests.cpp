...
static uint256 ComputeMerkleRootFromBranch(const uint256 &leaf, const std::vector<uint256> &vMerkleBranch,
                                           uint32_t nIndex) {
  uint256 hash = leaf;
...
static uint256 BlockBuildMerkleTree(const CBlock &block, bool *fMutated, std::vector<uint256> &vMerkleTree) {
  ...
  for (int nSize = block.vtx.size(); nSize > 1; nSize = (nSize + 1) / 2) {
    for (int i = 0; i < nSize; i += 2) {
      ...
      vMerkleTree.push_back(Hash(vMerkleTree[j + i].begin(), vMerkleTree[j + i].end(), vMerkleTree[j + i2].begin(),
                                 vMerkleTree[j + i2].end()));
    }
    j += nSize;
  }
...
static std::vector<uint256> BlockGetMerkleBranch(const CBlock &block, const std::vector<uint256> &vMerkleTree,
                                                 int nIndex) {
    ...
    for (int mutate = 0; mutate <= 3; mutate++) {
      ...
      if (mutate == 0) {
        for (int loop = 0; loop < std::min(ntx, 16); loop++) {
          ...
          // ones.
          int mtx = loop;
          if (ntx > 16) { mtx = InsecureRandRange(ntx); }
