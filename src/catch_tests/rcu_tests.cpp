...
struct RCUTest {
  ...
  static uint64_t hasSyncedTo(uint64_t syncRev) { return RCUInfos::infos.hasSyncedTo(syncRev); }

  static std::map<uint64_t, std::function<void()>> &getCleanups() { return RCUInfos::infos.cleanups; }
};
...
#define WAIT_FOR_STEP(step)                                                                                            \
  do {                                                                                                                 \
    cond.notify_all();                                                                                                 \
  } while (!cond.wait_for(lock, std::chrono::milliseconds(1), [&] { return otherstep == step; }))

void synchronize(std::atomic<RCUTestStep> &step, const std::atomic<RCUTestStep> &otherstep, Mutex &cs,
                 std::condition_variable &cond, std::atomic<uint64_t> &syncRev) {
...
void lockAndWaitForSynchronize(std::atomic<RCUTestStep> &step, const std::atomic<RCUTestStep> &otherstep, Mutex &cs,
                               std::condition_variable &cond, std::atomic<uint64_t> &syncRev) {
  assert(step == RCUTestStep::Init);
  WAIT_LOCK(cs, lock);
...
TEST_CASE("synchronize_test") {
  ...
  for (int i = 0; i < COUNT; i++) {
    ...
    std::thread tlock([&] { lockAndWaitForSynchronize(parentstep, childstep, cs, cond, syncRev); });

    std::thread tsync([&] { synchronize(childstep, parentstep, cs, cond, syncRev); });
...
TEST_CASE("cleanup_test") {
  ...
  BOOST_CHECK_EQUAL(RCUTest::getRevision(), RCUTest::getCleanups().begin()->first);
...
class RCURefTestItem {
  ...
  public:
  explicit RCURefTestItem(const std::function<void()> &fun) : cleanupfun(fun) {}
  ~RCURefTestItem() { cleanupfun(); }

  uint32_t getRefCount() const { return refcount.load(); }
};

TEST_CASE("rcuptr_test") {
...
TEST_CASE("rcuptr_operator_test") {
...
class RCURefMoveTestItem {
  ...
  public:
  explicit RCURefMoveTestItem(const std::function<void()> &fun) : cleanupfun(fun) {}
  ~RCURefMoveTestItem() { cleanupfun(); }

  void acquire() { throw std::runtime_error("RCUPtr incremented the refcount"); }
  void release() {
    RCULock::registerCleanup([this] { delete this; });
  }
};

TEST_CASE("move_rcuptr_test") {
  bool isDestroyed = false;

  // Check tat copy is failing.
  auto rcuptr1 = RCUPtr<RCURefMoveTestItem>::make([&] { isDestroyed = true; });
  ...
  auto ptrCopy = ptr;
  BOOST_CHECK_THROW(RCUPtr<RCURefMoveTestItem>::copy(ptr), std::runtime_error);
