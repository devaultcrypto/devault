...
static bool Verify(const CScript &scriptSig, const CScript &scriptPubKey, bool fStrict, ScriptError &err) {
  ...
  txTo.vout[0].nValue = Amount::min_amount();
...
TEST_CASE("sign") {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    for (int j = 0; j < 8; j++) {
      ...
      bool sigOK =
          CScriptCheck(output.scriptPubKey, output.nValue, CTransaction(txTo[i]), 0,
                       SCRIPT_VERIFY_P2SH | SCRIPT_VERIFY_STRICTENC | SCRIPT_ENABLE_SIGHASH_FORKID, false, txdata)();
      if (i == j) {
...
TEST_CASE("AreInputsStandard") {
  ...
  BOOST_CHECK(SignSignature(keystore, CTransaction(txFrom), txTo, 0, SigHashType().withForkId()));
  BOOST_CHECK(SignSignature(keystore, CTransaction(txFrom), txTo, 1, SigHashType().withForkId()));
  BOOST_CHECK(SignSignature(keystore, CTransaction(txFrom), txTo, 2, SigHashType().withForkId()));
  ...
  txTo.vin[3].scriptSig << OP_11 << OP_11 << std::vector<uint8_t>(oneAndTwo.begin(), oneAndTwo.end());
  txTo.vin[4].scriptSig << std::vector<uint8_t>(fifteenSigops.begin(), fifteenSigops.end());

  BOOST_CHECK(::AreInputsStandard(CTransaction(txTo), coins));
  // 22 P2SH sigops for all inputs (1 for vin[0], 6 for vin[3], 15 for vin[4]
  BOOST_CHECK_EQUAL(GetP2SHSigOpCount(CTransaction(txTo), coins, STANDARD_SCRIPT_VERIFY_FLAGS), 22U);
  ...
  txToNonStd1.vin[0].scriptSig << std::vector<uint8_t>(sixteenSigops.begin(), sixteenSigops.end());

  BOOST_CHECK(!::AreInputsStandard(CTransaction(txToNonStd1), coins));
  BOOST_CHECK_EQUAL(GetP2SHSigOpCount(CTransaction(txToNonStd1), coins, STANDARD_SCRIPT_VERIFY_FLAGS), 16U);
  // Check that no sigops show up when P2SH is not activated.
  ...
  txToNonStd2.vin[0].scriptSig << std::vector<uint8_t>(twentySigops.begin(), twentySigops.end());

  BOOST_CHECK(!::AreInputsStandard(CTransaction(txToNonStd2), coins));
  BOOST_CHECK_EQUAL(GetP2SHSigOpCount(CTransaction(txToNonStd2), coins, STANDARD_SCRIPT_VERIFY_FLAGS), 20U);
  // Check that no sigops show up when P2SH is not activated.
