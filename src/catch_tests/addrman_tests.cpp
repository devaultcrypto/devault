...
TEST_CASE("caddrinfo_get_tried_bucket") {
  BasicTestingSetup setup;
...
TEST_CASE("caddrinfo_get_new_bucket") {
  BasicTestingSetup setup;
  ...
  for (int j = 0; j < 4 * 255; j++) {
    CAddrInfo infoj = CAddrInfo(
        CAddress(ResolveService(std::to_string(250 + (j / 255)) + "." + std::to_string(j % 256) + ".1.1"), NODE_NONE),
        ResolveIP("251.4.1.1"));
...
TEST_CASE("addrman_selecttriedcollision") {
  BasicTestingSetup setup;
...
TEST_CASE("addrman_evictionworks") {
  BasicTestingSetup setup;
