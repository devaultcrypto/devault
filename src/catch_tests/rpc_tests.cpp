...
TEST_CASE("rpc_rawsign") {
  ...
  r = CallRPC(std::string("signrawtransactionwithkey ") + notsigned + " [" + privkey1 + "," + privkey2 + "] " +
              prevout);
  BOOST_CHECK(find_value(r.get_obj(), "complete").get_bool() == true);
}

TEST_CASE("rpc_rawsign_missing_amount") {
  ...
  try {
    ...
    if (std::string(e.what()).find("amount") != std::string::npos) { errorWasMissingAmount = true; }
  }
  BOOST_CHECK(exceptionThrownDueToMissingAmount == true);
  BOOST_CHECK(errorWasMissingAmount == true);
}

TEST_CASE("rpc_createraw_op_return") {
  BOOST_CHECK_NO_THROW(CallRPC("createrawtransaction "
                               "[{\"txid\":"
...
TEST_CASE("rpc_format_monetary_values") {
  BOOST_CHECK(ValueFromAmount(Amount::zero()).write() == "0.00000000");
  BOOST_CHECK(ValueFromAmount(Amount::min_amount()).write() == "0.00000001");
  ...
  BOOST_CHECK(ValueFromAmount(Amount(2099999999999990)).write() == "20999999.99999990");
  BOOST_CHECK(ValueFromAmount(Amount(2099999999999999)).write() == "20999999.99999999");
...
TEST_CASE("rpc_parse_monetary_values") {
  BOOST_CHECK_THROW(AmountFromValue(ValueFromString("-0.00000001")), UniValue);
  BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("0")), Amount::zero());
  BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("0.00000000")), Amount::zero());
  BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("0.00000001")), Amount::min_amount());
  ...
  BOOST_CHECK_EQUAL(
      AmountFromValue(ValueFromString("20999999.9999999")), Amount(2099999999999990);
      BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("20999999.99999999")), Amount(2099999999999999));
      ...
      BOOST_CHECK_EQUAL(AmountFromValue(ValueFromString("0.00000001000000")), Amount::min_amount()));
  // should fail
...
TEST_CASE("json_parse_errors") {
  ...
  BOOST_CHECK_EQUAL(AmountFromValue(ParseNonRFCJSONValue("0.00000000000000000000000000000000000001e+30 ")),
                    Amount::min_amount());
...
TEST_CASE("rpc_convert_values_generatetoaddress") {
  UniValue result;
  ...
  BOOST_CHECK_NO_THROW(result =
                           RPCConvertValues("generatetoaddress", {"1", "mkESjLZW66TmHhiFX8MCaBjrhZ543PPh9a", "9"}));
  BOOST_CHECK_EQUAL(result[0].get_int(), 1);
  BOOST_CHECK_EQUAL(result[1].get_str(), "mkESjLZW66TmHhiFX8MCaBjrhZ543PPh9a");
  BOOST_CHECK_EQUAL(result[2].get_int(), 9);

  BOOST_CHECK_NO_THROW(result =
                           RPCConvertValues("generatetoaddress", {"1", "mhMbmE2tE9xzJYCV9aNC8jKWN31vtGrguU", "9"}));
  BOOST_CHECK_EQUAL(result[0].get_int(), 1);
  BOOST_CHECK_EQUAL(result[1].get_str(), "mhMbmE2tE9xzJYCV9aNC8jKWN31vtGrguU");
  BOOST_CHECK_EQUAL(result[2].get_int(), 9);
}
