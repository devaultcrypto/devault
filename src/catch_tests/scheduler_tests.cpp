...
TEST_CASE("manythreads") {
  ...
  for (int i = 0; i < 100; ++i) {
    ...
    CScheduler::Function f = std::bind(&microTask, std::ref(microTasks), std::ref(counterMutex[whichCounter]),
...
TEST_CASE("schedule_every") {
  ...
  std::thread schedulerThread(std::bind(&CScheduler::serviceQueue, &scheduler));
