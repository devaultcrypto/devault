...
struct AvalancheTest {
  ...
  static std::vector<CInv> getInvsForNextPoll(const AvalancheProcessor &p) { return p.getInvsForNextPoll(false); }

  static NodeId getSuitableNodeToQuery(AvalancheProcessor &p) { return p.getSuitableNodeToQuery(); }
...
std::unique_ptr<CNode> ConnectNode(const Config &config, ServiceFlags nServices, PeerLogicValidation &peerLogic) {
  static NodeId id = 0;

  CAddress addr(ip(GetRandInt(0xffffffff)), NODE_NONE);
  std::unique_ptr<CNode> nodeptr(new CNode(id++, ServiceFlags(NODE_NETWORK), 0, INVALID_SOCKET, addr, 0, 0, CAddress(),
                                           "",
                                           /*fInboundIn=*/false));
  CNode &node = *nodeptr;
...
std::array<std::unique_ptr<CNode>, 8> ConnectNodes(const Config &config, AvalancheProcessor &p, ServiceFlags nServices,
...
TEST_CASE("block_register") {
  ...
  BOOST_CHECK_EQUAL(updates[0].getStatus(), AvalancheBlockUpdate::Status::Finalized);
  updates = {};
  ...
  BOOST_CHECK_EQUAL(updates.size(), 1);
  BOOST_CHECK(updates[0].getBlockIndex() == pindex);
  BOOST_CHECK_EQUAL(updates[0].getStatus(), AvalancheBlockUpdate::Status::Rejected);
  updates = {};
  ...
  BOOST_CHECK_EQUAL(updates.size(), 1);
  BOOST_CHECK(updates[0].getBlockIndex() == pindex);
  BOOST_CHECK_EQUAL(updates[0].getStatus(), AvalancheBlockUpdate::Status::Invalid);
  updates = {};
...
TEST_CASE("multi_block_register") {
  ...
  AvalancheTest::runEventLoop(p);
  BOOST_CHECK(p.registerVotes(avanodes[0]->GetId(), {round, 0, {AvalancheVote(0, blockHashA)}}, updates));
  BOOST_CHECK_EQUAL(updates.size(), 0);

  // Start voting on block B after one vote.
  AvalancheResponse resp{round + 1, 0, {AvalancheVote(0, blockHashB), AvalancheVote(0, blockHashA)}};
...
TEST_CASE("poll_and_response") {
  ...
  // 1. Too many results.
  resp = {round, 0, {AvalancheVote(0, blockHash), AvalancheVote(0, blockHash)}};
  AvalancheTest::runEventLoop(p);
  BOOST_CHECK(!p.registerVotes(avanodeid, resp, updates));
  BOOST_CHECK_EQUAL(updates.size(), 0);
  BOOST_CHECK_EQUAL(AvalancheTest::getSuitableNodeToQuery(p), avanodeid);
  ...
  resp = {AvalancheTest::getRound(p), 0, {AvalancheVote(0, blockHash), AvalancheVote(0, blockHash2)}};
  AvalancheTest::runEventLoop(p);
  BOOST_CHECK(!p.registerVotes(avanodeid, resp, updates));
  BOOST_CHECK_EQUAL(updates.size(), 0);
  BOOST_CHECK_EQUAL(AvalancheTest::getSuitableNodeToQuery(p), avanodeid);
  ...
  resp = {AvalancheTest::getRound(p), 0, {AvalancheVote(0, blockHash2), AvalancheVote(0, blockHash)}};
  AvalancheTest::runEventLoop(p);
  BOOST_CHECK(p.registerVotes(avanodeid, resp, updates));
  BOOST_CHECK_EQUAL(updates.size(), 0);
  BOOST_CHECK_EQUAL(AvalancheTest::getSuitableNodeToQuery(p), avanodeid);
...
TEST_CASE("poll_inflight_timeout") {
  ...
  for (int i = 0; i < 10; i++) {
    AvalancheResponse resp = {AvalancheTest::getRound(p), 0, {AvalancheVote(0, blockHash)}};
    AvalancheTest::runEventLoop(p);
...
TEST_CASE("quorum_diversity") {
  ...
  for (auto &pair : node_round_map) {
    ...
    BOOST_CHECK(p.registerVotes(nodeid, {r, 0, {AvalancheVote(0, blockHash)}}, updates));
    BOOST_CHECK_EQUAL(p.getConfidence(pindex), confidence);
  }

  BOOST_CHECK(p.registerVotes(firstNodeId, {round, 0, {AvalancheVote(0, blockHash)}}, updates));
...
TEST_CASE("event_loop") {
  ...
  for (int i = 0; i < 1000; i++) {
    ...
    std::this_thread::sleep_for(std::chrono::milliseconds(1));
    if (AvalancheTest::getRound(p) != queryRound) { break; }
  }
  ...
  auto queryTime = std::chrono::steady_clock::now() + std::chrono::milliseconds(100);

  std::vector<AvalancheBlockUpdate> updates;
  p.registerVotes(nodeid, {queryRound, 100, {AvalancheVote(0, blockHash)}}, updates);
  for (int i = 0; i < 1000; i++) {
