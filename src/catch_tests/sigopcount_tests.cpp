...
TEST_CASE("GetSigOpCount") {
  ...
  BOOST_CHECK_EQUAL(p2sh.GetSigOpCount((STANDARD_SCRIPT_VERIFY_FLAGS & ~SCRIPT_VERIFY_CHECKDATASIG_SIGOPS), scriptSig2),
                    3U);
  BOOST_CHECK_EQUAL(p2sh.GetSigOpCount(STANDARD_SCRIPT_VERIFY_FLAGS | SCRIPT_VERIFY_CHECKDATASIG_SIGOPS, scriptSig2),
                    3U);
 ...
 */
void BuildTxs(CMutableTransaction &spendingTx, CCoinsViewCache &coins, CMutableTransaction &creationTx,
  ...
  creationTx.vout[0].nValue = Amount::min_amount();
  ...
  spendingTx.vout[0].nValue = Amount::min_amount();
  spendingTx.vout[0].scriptPubKey = CScript();

  AddCoins(coins, CTransaction(creationTx), 0);
}

TEST_CASE("GetTxSigOpCost") {
  BasicTestingSetup setup;
  ...
  // Multisig script (legacy counting)
  {
    ...
    BOOST_CHECK_EQUAL(GetTransactionSigOpCount(CTransaction(creationTx), coins, SCRIPT_VERIFY_NONE),
...
TEST_CASE("test_consensus_sigops_limit") {
  BasicTestingSetup setup;
...
TEST_CASE("test_max_sigops_per_tx") {
  BasicTestingSetup setup;
  ...
  tx.vout.resize(1);
  tx.vout[0].nValue = Amount::min_amount();
