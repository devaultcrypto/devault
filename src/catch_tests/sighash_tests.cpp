...
static uint256 SignatureHashOld(CScript scriptCode, const CTransaction &txTo, unsigned int nIn, uint32_t nHashType) {
...
static void RandomTransaction(CMutableTransaction &tx, bool fSingle) {
  ...
  for (int in = 0; in < ins; in++) {
    ...
    txin.nSequence = (InsecureRandBool()) ? insecure_rand() : (unsigned int)-1;
  }
...
TEST_CASE("sighash_test") {
  ...
  for (int i = 0; i < nRandomTests; i++) {
    ...
    uint256 shold = SignatureHash(scriptCode, CTransaction(txTo), nIn, sigHashType, Amount::zero(), nullptr, 0);
    BOOST_CHECK(shold == shref);

    // Check the impact of the forkid flag.
    uint256 shreg = SignatureHash(scriptCode, CTransaction(txTo), nIn, sigHashType, Amount::zero());
    ...
    uint256 shrep = SignatureHash(scriptCode, CTransaction(txTo), nIn, sigHashType, Amount::zero(), nullptr,
                                  SCRIPT_ENABLE_SIGHASH_FORKID | SCRIPT_ENABLE_REPLAY_PROTECTION);
    uint32_t newForValue = 0xff0000 | ((nHashType >> 8) ^ 0xdead);
    uint256 manualshrep =
        SignatureHash(scriptCode, CTransaction(txTo), nIn, sigHashType.withForkValue(newForValue), Amount::zero());
    BOOST_CHECK(shrep == manualshrep);

    // Replay protection works even if the hash is of the form 0xffxxxx
    uint256 shrepff =
        SignatureHash(scriptCode, CTransaction(txTo), nIn, sigHashType.withForkValue(newForValue), Amount::zero(),
                      nullptr, SCRIPT_ENABLE_SIGHASH_FORKID | SCRIPT_ENABLE_REPLAY_PROTECTION);
    ...
    uint256 shrepabcdef =
        SignatureHash(scriptCode, CTransaction(txTo), nIn, sigHashType.withForkValue(0xabcdef), Amount::zero(), nullptr,
                      SCRIPT_ENABLE_SIGHASH_FORKID | SCRIPT_ENABLE_REPLAY_PROTECTION);
    uint256 manualshrepabcdef =
        SignatureHash(scriptCode, CTransaction(txTo), nIn, sigHashType.withForkValue(0xff1342), Amount::zero());
...
TEST_CASE("sighash_from_data") {
  ...
  for (size_t idx = 0; idx < tests.size(); idx++) {
    ...
    uint256 shreg = SignatureHash(scriptCode, *tx, nIn, sigHashType, Amount::zero());
    BOOST_CHECK_MESSAGE(shreg.GetHex() == sigHashRegHex, strTest);

    uint256 shold = SignatureHash(scriptCode, *tx, nIn, sigHashType, Amount::zero(), nullptr, 0);
    BOOST_CHECK_MESSAGE(shold.GetHex() == sigHashOldHex, strTest);

    uint256 shrep = SignatureHash(scriptCode, *tx, nIn, sigHashType, Amount::zero(), nullptr,
                                  SCRIPT_ENABLE_SIGHASH_FORKID | SCRIPT_ENABLE_REPLAY_PROTECTION);
