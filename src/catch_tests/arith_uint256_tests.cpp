...
inline arith_uint256 arith_uint256V(const std::vector<uint8_t> &vch) { return UintToArith256(uint256(vch)); }

const uint8_t R1Array[] = "\x9c\x52\x4a\xdb\xcf\x56\x11\x12\x2b\x29\x12\x5e\x5d\x35\xd2\xd2"
                          "\x22\x81\xaa\xb5\x33\xf0\x08\x32\xd5\x56\xb1\xf9\xea\xe5\x1d\x7d";
const char R1ArrayHex[] = "7D1DE5EAF9B156D53208F033B5AA8122D2d2355d5e12292b121156cfdb4a529c";
// R1L equals roughly R1Ldouble * 2^256
const double R1Ldouble = 0.4887374590559308955;
const arith_uint256 R1L = arith_uint256V(std::vector<uint8_t>(R1Array, R1Array + 32));
const uint64_t R1LLow64 = 0x121156cfdb4a529cULL;

const uint8_t R2Array[] = "\x70\x32\x1d\x7c\x47\xa5\x6b\x40\x26\x7e\x0a\xc3\xa6\x9c\xb6\xbf"
                          "\x13\x30\x47\xa3\x19\x2d\xda\x71\x49\x13\x72\xf0\xb4\xca\x81\xd7";
const arith_uint256 R2L = arith_uint256V(std::vector<uint8_t>(R2Array, R2Array + 32));

const char R1LplusR2L[] = "549FB09FEA236A1EA3E31D4D58F1B1369288D204211CA751527CFC175767850C";

const uint8_t ZeroArray[] = "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                            "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
const arith_uint256 ZeroL = arith_uint256V(std::vector<uint8_t>(ZeroArray, ZeroArray + 32));

const uint8_t OneArray[] = "\x01\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00"
                           "\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00";
const arith_uint256 OneL = arith_uint256V(std::vector<uint8_t>(OneArray, OneArray + 32));

const uint8_t MaxArray[] = "\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff"
                           "\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff\xff";
const arith_uint256 MaxL = arith_uint256V(std::vector<uint8_t>(MaxArray, MaxArray + 32));
...
void shiftArrayRight(uint8_t *to, const uint8_t *from, unsigned int arrayLength, unsigned int bitsToShift) {
...
void shiftArrayLeft(uint8_t *to, const uint8_t *from, unsigned int arrayLength, unsigned int bitsToShift) {
  for (unsigned int T = 0; T < arrayLength; ++T) {
    if (T >= bitsToShift / 8) {
      unsigned int F = T - bitsToShift / 8;
      to[T] = from[F] << (bitsToShift % 8);
...
TEST_CASE("shifts") {
  ...
  for (unsigned int i = 0; i < 256; ++i) {
    ...
    BOOST_CHECK(arith_uint256V(std::vector<uint8_t>(TmpArray, TmpArray + 32)) == (OneL << i));
    TmpL = OneL;
    TmpL <<= i;
    ...
    BOOST_CHECK(arith_uint256V(std::vector<uint8_t>(TmpArray, TmpArray + 32)) == (R1L << i));
    TmpL = R1L;
    TmpL <<= i;
    ...
    BOOST_CHECK(arith_uint256V(std::vector<uint8_t>(TmpArray, TmpArray + 32)) == (MaxL << i));
    TmpL = MaxL;
    TmpL <<= i;
...
TEST_CASE("unaryOperators") {
  ...
  BOOST_CHECK(arith_uint256V(std::vector<uint8_t>(TmpArray, TmpArray + 32)) == (~R1L));
...
#define CHECKBITWISEOPERATOR(_A_, _B_, _OP_)                                                                           \
  ...
  BOOST_CHECK(arith_uint256V(std::vector<uint8_t>(TmpArray, TmpArray + 32)) == (_A_##L _OP_ _B_##L));
