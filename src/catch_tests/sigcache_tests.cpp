 ...
 */
static const uint32_t TEST_VARIANT_FLAGS = SCRIPT_ENABLE_BLS;
...
class TestCachingTransactionSignatureChecker {
  ...
  public:
  TestCachingTransactionSignatureChecker(CachingTransactionSignatureChecker &checkerarg) { pchecker = &checkerarg; }

  inline bool VerifyAndStore(const std::vector<uint8_t> &vchSig, const CPubKey &pubkey, const uint256 &sighash,
                             uint32_t flags) {
    return pchecker->VerifySignature(vchSig, pubkey, sighash, flags);
  }

  inline bool IsCached(const std::vector<uint8_t> &vchSig, const CPubKey &pubkey, const uint256 &sighash,
                       uint32_t flags) {
...
TEST_CASE("sig_pubkey_hash_variations") {
  /**
  ...
  CDataStream stream(ParseHex("010000000122739e70fbee987a8be1788395a2f2e6ad18ccb7ff611cd798071539"
  ...
  CachingTransactionSignatureChecker checker(&dummyTx, 0, Amount(), true, txdata);
...
TEST_CASE("flag_invariants") {
  ...
  for (int n = 0; n < 16; n++) {
    ...
    // insert into cache
    BOOST_CHECK(testChecker.VerifyAndStore(sig, pubkey1, hashMsg, base_flags));
    // check that it's in
    BOOST_CHECK(testChecker.IsCached(sig, pubkey1, hashMsg, base_flags));
    ...
    for (uint32_t flag = 1; flag; flag <<= 1) {
      ...
      if (TEST_VARIANT_FLAGS & flag) {
        ...
        BOOST_CHECK_MESSAGE(!hit, n << " bad cache hit 0x" << std::hex << base_flags << " ^ 0x" << flag);
      } else if (TEST_INVARIANT_FLAGS & flag) {
        // if it's in TEST_INVARIANT_FLAGS, we must hit cache
        BOOST_CHECK_MESSAGE(hit, n << " bad cache miss 0x" << std::hex << base_flags << " ^ 0x" << flag);
      } else {
