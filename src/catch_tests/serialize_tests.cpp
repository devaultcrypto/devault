...
//????
#define BOOST_CHECK_EXCEPTION(a, b, c) CHECK_THROWS_AS(a, b)
...
TEST_CASE("floats") {
  ...
  BOOST_CHECK(Hash(ss.begin(), ss.end()) == uint256S("8e8b4cf3e4df8b332057e3e23af42ebc663b61e0495d5e7e32d85"
                                                     "099d7f3fe0c"));
...
TEST_CASE("doubles") {
  ...
  BOOST_CHECK(Hash(ss.begin(), ss.end()) == uint256S("43d0c82591953c4eafe114590d392676a01585d25b25d433557f0"
                                                     "d7878b23f96"));
...
static bool isTooLargeException(const std::ios_base::failure &ex) {
  std::ios_base::failure expectedException("ReadCompactSize(): size too large");
...
TEST_CASE("compactsize") {
  ...
  for (i = 1; i <= MAX_SIZE; i *= 2) {
    ...
    std::stringstream m;
    std::stringstream m1;
    m << "decoded:" << j << " expected:" << (i - 1) << "\n";
    m1 << "decoded:" << j << " expected:" << i << "\n";
    BOOST_CHECK_MESSAGE((i - 1) == j, m.str());
    j = ReadCompactSize(ss);
    BOOST_CHECK_MESSAGE(i == j, m1.str());
  }
