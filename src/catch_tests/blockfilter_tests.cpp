...
TEST_CASE("blockfilter_basic_test") {
  ...
  included_scripts[1] << OP_DUP << OP_HASH160 << std::vector<uint8_t>(1, 20) << OP_EQUALVERIFY << OP_CHECKSIG;

  // Third is an output on in a second transaction.
  included_scripts[2] << OP_1 << std::vector<uint8_t>(2, 33) << OP_1 << OP_CHECKMULTISIG;
  ...
  tx_1.vout.emplace_back(100 * Amount::min_amount(), included_scripts[0]);
  tx_1.vout.emplace_back(200 * Amount::min_amount(), included_scripts[1]);

  CMutableTransaction tx_2;
  tx_2.vout.emplace_back(300 * Amount::min_amount(), included_scripts[2]);
  tx_2.vout.emplace_back(0 * Amount::min_amount(), excluded_scripts[0]);
  // Script is empty
  tx_2.vout.emplace_back(400 * Amount::min_amount(), excluded_scripts[2]);
  ...
  block_undo.vtxundo.back().vprevout.emplace_back(CTxOut(500 * Amount::min_amount(), included_scripts[3]), 1000, true);
  block_undo.vtxundo.back().vprevout.emplace_back(CTxOut(600 * Amount::min_amount(), included_scripts[4]), 10000,
                                                  false);
  block_undo.vtxundo.back().vprevout.emplace_back(CTxOut(700 * Amount::min_amount(), excluded_scripts[2]), 100000,
                                                  false);
...
TEST_CASE("blockfilters_json_test") {
  UniValue json;
  std::string json_data(json_tests::blockfilters, json_tests::blockfilters + sizeof(json_tests::blockfilters));
  ...
  for (size_t i = 0; i < tests.size(); i++) {
    ...
    for (size_t ii = 0; ii < prev_scripts.size(); ii++) {
      std::vector<uint8_t> raw_script = ParseHex(prev_scripts[ii].get_str());
      CTxOut txout(0 * Amount::min_amount(), CScript(raw_script.begin(), raw_script.end()));
