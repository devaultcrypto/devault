...
static void Correct_Queue_range(std::vector<size_t> range) {
  ...
  for (const size_t i : range) {
    ...
    if (FakeCheckCheckCompletion::n_calls != i) {
      ...
      std::cout << "Failure on trial " << i << " expected, got " << FakeCheckCheckCompletion::n_calls << "\n";
    }
  }
 ...
 */
TEST_CASE("test_CheckQueue_Correct_Zero") {
 ...
 */
TEST_CASE("test_CheckQueue_Correct_One") {
  std::vector<size_t> range;
 ...
 */
TEST_CASE("test_CheckQueue_Correct_Max") {
  std::vector<size_t> range;
 ...
 */
TEST_CASE("test_CheckQueue_Correct_Random") {
  std::vector<size_t> range;
  range.reserve(100000 / 1000);
  for (size_t i = 2; i < 100000;
       i += std::max((size_t)1, (size_t)InsecureRandRange(std::min((size_t)1000, ((size_t)100000) - i))))
    range.push_back(i);
  Correct_Queue_range(range);
}

/** Test that failing checks are caught */
TEST_CASE("test_CheckQueue_Catches_Failure") {
...
TEST_CASE("test_CheckQueue_Recovers_From_Failure") {
  auto fail_queue = std::unique_ptr<Failing_Queue>(new Failing_Queue{QUEUE_BATCH_SIZE});
  std::vector<std::thread> tg;
  for (auto x = 0; x < nScriptCheckThreads; ++x) {
    tg.emplace_back(std::thread([&] { fail_queue->Thread(); }));
...
TEST_CASE("test_CheckQueue_UniqueCheck") {
...
TEST_CASE("test_CheckQueue_Memory") {
...
TEST_CASE("test_CheckQueue_FrozenCleanup") {
...
TEST_CASE("test_CheckQueueControl_Locks") {
