...
static void CheckSigHashType(SigHashType t, BaseSigHashType baseType, bool isDefined, uint32_t forkValue,
...
TEST_CASE("sighash_construction_test") {
  ...
  std::set<BaseSigHashType> baseTypes{BaseSigHashType::UNSUPPORTED, BaseSigHashType::ALL, BaseSigHashType::NONE,
    ...
    for (uint32_t forkValue : forkValues) {
      for (bool hasForkId : forkIdFlagValues) {
        for (bool hasAnyoneCanPay : anyoneCanPayFlagValues) {
          const SigHashType t =
              SigHashType().withBaseType(baseType).withForkValue(forkValue).withForkId(hasForkId).withAnyoneCanPay(
                  hasAnyoneCanPay);
          ...
          CheckSigHashType(t.withForkId(hasForkId), baseType, isDefined, forkValue, hasForkId, hasAnyoneCanPay);
          CheckSigHashType(t.withForkId(!hasForkId), baseType, isDefined, forkValue, !hasForkId, hasAnyoneCanPay);
...
TEST_CASE("sighash_serialization_test") {
