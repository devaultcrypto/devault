...
void SetupServerArgs() {
    ...
    gArgs.AddArg("-seedphrase=<words>", _("Specify 12-word seed phrase for wallet on 1st run (only)"),
                 false,OptionsCategory::OPTIONS);
   ...
   gArgs.AddArg("-maxmempool=<n>",
    ...
    gArgs.AddArg(
        ...
        false, OptionsCategory::OPTIONS);
#endif
    gArgs.AddArg("-addressindex", strprintf(_("Maintain a full address index, used to query for the balance,"
                                              " txids and unspent outputs for addresses (default: %u)"), DEFAULT_ADDRESSINDEX), false, OptionsCategory::OPTIONS);
#ifdef EXTRA_INDEXES    
    gArgs.AddArg("-timestampindex", strprintf(_("Maintain a timestamp index for block hashes,"
                                                " used to query blocks hashes by a range of timestamps (default: %u)"), DEFAULT_TIMESTAMPINDEX));
    gArgs.AddArg("-spentindex", strprintf(_("Maintain a full spent index, used to query the spending txid "
