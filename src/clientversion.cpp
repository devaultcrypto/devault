 ...
 * * first, for autoconf builds, HAVE_BUILD_INFO is defined, 
 *   which will include build.h, a file that is
 *   generated by the build environment, possibly containing the output
 *   of git-describe in a macro called BUILD_DESC
 * * otherwise, cmake will provide the equivalent info
 *
 * * then, these options exist for determining CLIENT_BUILD:
 *   * if BUILD_DESC is defined, use that literally (output of git-describe)
 *   * otherwise, use BUILD_SUFFIX which should be loke v[maj].[min].[rev].[build]-[gitsha]
 * finally CLIENT_VERSION_SUFFIX is added
 */

//! First, include build.h if requested
//! This is defined by configure.ac but not CMake
...
#else
// Start Cmake build processing

#ifdef BUILD_SUFFIX_DEFINED
#undef BUILD_DESC

#define BUILD_DESC_WITH_SUFFIX(maj, min, rev, build, suffix)            \
    "v" DO_STRINGIZE(maj) "." DO_STRINGIZE(min) "." DO_STRINGIZE(              \
        rev) "." DO_STRINGIZE(build) "-" suffix
