...
bool VerifyAggregate(const uint256 &hash, const std::vector<uint8_t> &aggSig, const std::vector<uint8_t> &aggPubKey) {
  try {
    // Unserialize from CPubKey back to PublicKey
  ...
  catch (...) { return false; } // Don't crash!
}
...
std::vector<uint8_t> AggregateSigForMessages(std::map<uint256, CKey> &keys_plus_hash) {
  ...
  for (const auto &kph : keys_plus_hash) {
    ...
    /*
    {
      ...
      std::cout << "hash = " << kph.first.ToString()
                << " addr = " << EncodeDestination(kph.second.GetPubKeyForBLS().GetBLSKeyID()) << " "
                << " pub = " << HexStr(ToByteVector(kph.second.GetPubKeyForBLS())) << " "
                << " secret addr = " << EncodeSecret(kph.second) << " "
                << " secret = " << HexStr(kph.second.begin(),kph.second.end()) << " "
                << "sig = " << HexStr(vchSig) << "\n";
    }
    */
  }
  ...
  try {
    Signature aggSig = Signature::Aggregate(sigs);
    aggSigsAsBytes = aggSig.Serialize();
    //std::cout << "AggSig = " << HexStr(aggSigsAsBytes) << "\n";
...
std::vector<uint8_t> MakeAggregateSigsForMessages(const std::vector<uint256> &msgs,
  ...
  std::vector<uint8_t> aggSigsAsBytes;
  int N = aggSigs.size();
  int MsgsPerN = msgs.size()/N;
    
  int inc = 0;
  for (int i=0;i<N;i++) {
      std::vector<bls::AggregationInfo> infos;
      for (int j=0;j<MsgsPerN;j++) {
          bls::PublicKey pk = bls::PublicKey::FromBytes(pubkeys[inc].data());
          auto info = bls::AggregationInfo::FromMsgHash(pk, msgs[inc++].begin());
          infos.push_back(info);
      }
      bls::Signature sig = bls::Signature::FromBytes(aggSigs[i].data());
      bls::AggregationInfo mergedInfo = bls::AggregationInfo::MergeInfos(infos);
      sig.SetAggregationInfo(mergedInfo);
      sigs.push_back(sig);
  }
  try {
