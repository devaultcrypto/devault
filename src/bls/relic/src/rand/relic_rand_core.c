...
#if RELICRAND == UDEV || RELICSEED == DEV || RELICSEED == UDEV
 ...
 */
#if RELICSEED == DEV
#define RELICRAND_PATH		"/dev/random"
#else
#define RELICRAND_PATH		"/dev/urandom"
#endif
...
void rand_init(void) {
	...
	*fd = open(RELICRAND_PATH, O_RDONLY);
	if (*fd == -1) {
		THROW(ERR_NO_FILE);
	}
#else
...
#elif RELICSEED == DEV || RELICSEED == UDEV
	int fd, c, l;
