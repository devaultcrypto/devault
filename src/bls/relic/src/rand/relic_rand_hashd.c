...
static void rand_gen(uint8_t *out, int out_len) {
	...
	uint8_t hash[RLC_MD_LEN], data[(RELICRAND_SIZE - 1)/2];
	ctx_t *ctx = core_get();

	/* data = V */
	memcpy(data, ctx->rand + 1, (RELICRAND_SIZE - 1)/2);
	for (int i = 0; i < m; i++) {
		/* w_i = Hash(data) */
		...
		out_len -= RLC_MD_LEN;
		/* data = data + 1 mod 2^b. */
		rand_inc(data, (RELICRAND_SIZE - 1)/2, 1);
	}
}
...
void rand_bytes(uint8_t *buf, int size) {
	uint8_t hash[RLC_MD_LEN];
	int carry, len  = (RELICRAND_SIZE - 1)/2;
	ctx_t *ctx = core_get();
