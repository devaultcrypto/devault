...
void CTxMemPool::addAddressIndex(const CTxMemPoolEntry &entry, const CCoinsViewCache &view)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        if (prevout.scriptPubKey.IsPayToScriptHash()) {
            ...
            CMempoolAddressDelta delta(entry.GetTime(), -prevout.nValue, input.prevout.GetTxId(), input.prevout.GetN());
 ...
 bool CTxMemPool::getAddressIndex(std::vector<CashAddrContent> &addresses,
                                 std::vector<std::pair<CMempoolAddressDeltaKey, CMempoolAddressDelta> > &results)
{
    LOCK(cs);
    for (const auto& it : addresses) {
        addressDeltaMap::iterator ait = mapAddress.lower_bound(CMempoolAddressDeltaKey(it.type,uint160(it.hash)));
        while (ait != mapAddress.end() && (*ait).first.addressBytes == uint160(it.hash) && (*ait).first.type == it.type) {
